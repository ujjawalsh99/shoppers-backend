...
const userData = [
  {
    customerEmail: "hello123@gmail.com",
    customerName: "Hello",
...
const productData = [
  ...
  },
  {
    ...
    pDescription:
      "A luxurious and comfortable sofa set by Grihshobha furniture makers",
    pRating: 4.8,
    pCategory: "Furniture",
    price: 12500,
    color: "Dark grey",
...
create.setupDBproduct = () => {
  return collection.getProductCollection().then((userColl) => {
    return userColl.deleteMany().then((data) => {
      if(data)
      return userColl.insertMany(productData).then((result) => {
        if (result && result.length > 0) return result.length;
        else return null;
      });
    });
